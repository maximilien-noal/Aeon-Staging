    ...
    internal sealed class MemoryBlockList : IEnumerable<MemoryControlBlock>
    {
        ...
        public bool? Reallocate(ushort segment, ushort newSize)
        {
            ...
            if(newSize < block.Length)
            {
                ...
                newBlock.IsLast = false;
                newBlock.Length = (ushort)(block.Length - newSize - 1);
        ...
        private void NormalizeTail()
        {
            ...
            if(spaceRemaining > 0)
            {
                ...
                else
                {
                    ...
                    newBlock.Length = (ushort)(spaceRemaining - 1);
                    newBlock.IsLast = true;
                    newBlock.Write(memory);
        ...
        private void Initialize()
        {
            ...
            var variableBlock = new MemoryControlBlock(EnvironmentVariableSegment, 0x0128, string.Empty);
            variableBlock.Length = EnvironmentBlockSize;
        ...
        private MemoryControlBlock FindNextAllocationBlock(ushort size)
        {
            ...
            {
            case AllocationStrategy.LowFirstFit:
            case AllocationStrategy.HighLowFirstFit:
                return (from mcb in this.FreeBlocks
                        where mcb.Length >= size
                        select mcb).FirstOrDefault();

            case AllocationStrategy.LowLastFit:
            case AllocationStrategy.HighLowLastFit:
                return (from mcb in this.FreeBlocks
                        where mcb.Length >= size
                        select mcb).LastOrDefault();

            case AllocationStrategy.LowBestFit:
            case AllocationStrategy.HighLowBestFit:
                return (from mcb in this.FreeBlocks
                        where mcb.Length >= size
